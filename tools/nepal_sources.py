...
def render_sources(data, analysis):
    ...
    for a in analysis['articles']:
        ...
        cards.append(f'''<details class="source-analysis" id="analysis-{e(a['id'])}" data-source-analysis><summary><span>{e(a['category'])}</span><strong>{e(a['title'])}</strong><small>한국어 해설 펼치기</small></summary><div class="analysis-body"><h4>원문에서 확인한 내용</h4><ul>{evidence}</ul><div class="analysis-meaning"><h4>한국팀 브리핑에 적용하기 <small>AI환경연구소 해석</small></h4><p>{e(a['meaning'])}</p></div><p class="analysis-limit"><b>자료의 범위</b> {e(a['limits'])}</p><details class="analysis-provenance"><summary>근거 문서·기준일 확인</summary><ul>{provenance}</ul><p>한국어 분석 갱신 {e(analysis['updatedOn'])}. 현장 상황의 기준 시점은 각 원문의 날짜·시각을 따릅니다.</p></details><a class="analysis-top" href="#korean-source-library">한글 원문 해설 목록으로 ↑</a></div></details>''')
