...
def ref(key, page=None):
    s=sources[key]
    url=s['url']+(f'#page={page}' if page else '')
    return f'<a class="brief-source" href="{e(url)}" target="_blank" rel="noopener noreferrer">{e(s["name"])}{(" · "+e(s["date"])) if s["date"] else ""} ↗</a>'
...
contacts=''.join(f'<article class="contact-card"><h3>{e(name)}</h3><small>{e(org)}</small><a class="contact-number" href="tel:{dial}">{number}</a><small>{e(scope)}</small><p>{e(note)}</p><a class="brief-source" href="{e(url)}" target="_blank" rel="noopener noreferrer">공식 출처 ↗</a></article>' for name,org,number,dial,scope,note,url in contact_items)
...
content='''
  ...
  <section id="responder-safety" class="section rescue-section" aria-labelledby="safety-title"><div class="section-head"><div><span class="section-no">대원 안전·접근</span><h2 id="safety-title">이번 홍수에서 특히 볼 위험 요소</h2><p>아래는 사고 당시 보고된 위험과 그에 따른 검토사항입니다. 현재 위험 등급·철수 수위·차량 운행 가능 여부는 제공하지 않습니다.</p></div></div>
    <div class="risk-grid">
      ...
      <article><span>지상 접근</span><h3>도로·교량과 이동 구간</h3><p>라수와의 도로·교량 피해가 보고됐고 9월 5일 부분 통제 공지가 올라왔습니다. 위성 영상만으로 교량 하중·차종별 통행을 판단할 수 없습니다.</p><p class="risk-meaning"><b>대원 브리핑에 포함:</b> 구간별 통제와 확인 시각, 차량 종류, 지휘부가 승인한 접근·복귀 계획.</p>__ROADS__</article>
  ...
  <section id="emergency-contacts" class="section rescue-section" aria-labelledby="contact-title"><div class="section-head"><div><span class="section-no">지휘·의료·한국 측 연락망</span><h2 id="contact-title">업무에 맞는 조정 창구</h2><p>공개 연락처 확인일 2026.09.05. 단축번호는 네팔 내 사용 기준이며, 실제 연결·가용 자원은 확인하지 않았습니다.</p></div></div><div class="contact-grid responder-contacts">__CONTACTS__</div>
    <details><summary>보조 연락처와 공식 상황판</summary><p>경찰 <a href="tel:100">100</a> · 구급차 <a href="tel:102">102</a> · 네팔적십자 <a href="tel:1130">1130</a> · 라수와 DEOC 대체 <a href="tel:+97710540019">+977-10-540019</a></p><p><a href="https://www.dhm.gov.np/">DHM 기상·홍수 안내 ↗</a> · <a href="https://daonuwakot.moha.gov.np/">누와코트 지구청 ↗</a> · <a href="https://heoc.mohp.gov.np/">HEOC 보건 상황보고 ↗</a></p><p class="field-note"><a href="./data/field-contacts.json">보조 연락처 출처 기록</a>. 일반 신고 번호와 구조대 임무 조정 창구를 구분해 사용하세요.</p></details>
  </section>
  <section id="field-support" class="section rescue-section" aria-labelledby="field-title"><div class="section-head"><div><span class="section-no">한국어 현장 참고자료</span><h2 id="field-title">대원이 들고 갈 브리핑</h2><p>지역·위험·의료·연락망을 사진 없는 한국어 문서로 저장할 수 있습니다. 저장 뒤에도 경보·도로·병상은 현지에서 새로 확인해야 합니다.</p></div></div>
    ...
    <p><a href="__KOREAN_GUIDE__" target="_blank" rel="noopener noreferrer">INSARAG 2020 지침 한국어판 PDF · 약 18MB ↗</a></p><p class="field-note">공식 안내 페이지는 2020판을 현행으로 소개하며 새 지침은 2027년 1월 1일 발효 예정이라고 안내합니다. 현장에서는 소속기관이 적용하는 지침을 따르세요. __INSARAG__</p>
...
'''
for key,value in {'__REGIONS__':'\n'.join(region_cards),'__ICIMOD__':ref('icimod'),'__ROADS__':ref('roads'),'__HEOC3__':ref('heoc',3),'__HEOC1__':ref('heoc',1),'__GUIDE__':ref('field-guide',8),'__MEDICAL__':medical,'__CONTACTS__':contacts,'__KOREAN_GUIDE__':data['koreanGuideUrl'],'__INSARAG__':ref('insarag'),'__SOURCES__':source_list}.items():content=content.replace(key,value)
